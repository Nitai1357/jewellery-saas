...
export default function CustomerShopPage() {
  ...
  const theme = getShopTheme(id as string);
  ...
  useEffect(() => {
    if (!id) return;
    const unsubUser = onSnapshot(doc(db, "users", id as string), (snap) => { if (snap.exists()) setShopInfo(snap.data()); });
    const unsubRates = onSnapshot(doc(db, "prices", id as string), (snap) => { if (snap.exists()) setRates(snap.data()); });
    const qBanners = query(collection(db, "banners"), where("ownerId", "==", id));
    const unsubBanners = onSnapshot(qBanners, (snap) => setBanners(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })).reverse()));
    
    const qProducts = query(collection(db, "products"), where("ownerId", "==", id), where("isTopSeller", "==", true), limit(10));
  ...
  const handleAddToCart = (prod: any) => {
    ...
    if(existingIndex > -1) {
      ...
      cart.push({ ...prod, livePrice: finalPrice, quantity: 1, shopId: id });
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-pink-100 flex flex-col font-sans text-zinc-900 overflow-x-hidden">
      ...
      {cartCount > 0 && (
        <button onClick={() => router.push(`/checkout?shopId=${id}`)} className={`fixed bottom-6 right-6 z-50 ${theme.bg} ${theme.textOnBg} ${theme.hover} p-4 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.15)] transition-all transform hover:scale-105 flex items-center justify-center`}>
          <ShoppingBag size={22} />
      ...
      <div className="px-6 md:px-10 pb-20 max-w-6xl mx-auto w-full pt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
          {categories.map((cat, idx) => (
            <Link key={idx} href={`/shop/${id}/category/${cat.name}`}>
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col items-center gap-6 mb-10 w-full">
            ...
            <div className="flex flex-row flex-wrap justify-center items-center gap-2 md:gap-4 w-full px-2">
              <Link href={`/shop/${id}/track`} className="flex-1 min-w-[100px] bg-white/80 border-2 border-pink-200 hover:border-pink-400 hover:bg-pink-50 text-zinc-800 px-2 py-3 rounded-xl text-[8px] md:text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5 shadow-sm">
                📦 Track
              </Link>
              {shopInfo?.whatsapp && (
